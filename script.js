...
function animateTrainRealtime(schedule) {
  ...
  for (let i = 0; i < schedule.stops.length - 1; i++) {
    ...
    if (fromIdx === -1 || toIdx === -1 || fromIdx === toIdx) {
      console.warn(`Jalur tidak valid: ${schedule.stops[i]} → ${schedule.stops[i + 1]}`);
      continue;
    }

    let pathSegment;
    if (fromIdx < toIdx) {
      pathSegment = allLatLngs.slice(fromIdx, toIdx + 1);
    } else {
      pathSegment = allLatLngs.slice(toIdx, fromIdx + 1).reverse();
    }

    if (pathSegment.length < 2) {
      console.warn(`Segmen terlalu pendek antara ${schedule.stops[i]} dan ${schedule.stops[i + 1]}`);
      continue;
    }

    segmentCoords.push(pathSegment);
  ...
  function update() {
    const now2 = new Date();
    if (now2 - times[times.length - 1] > 10 * 60 * 1000) {
      marker.remove();
      delete activeMarkers[trainId];
      return;
    }
